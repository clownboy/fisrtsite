var vm = new Vue({
    ...
    methods: {
        emailSet(){
          ...
          url = 'http://127.0.0.1:8000/emailset'
        ...
        passwordSet(){
          ...
          if(srt(password).length < 6 ){
              ...
              url = 'http://127.0.0.1:8000/passwordset'
              var formData = new FormData()
        ...
        talk(){
          ...
          this.$http.post(url, formData, {
              ...
              if (res.ok) {
                  window.location.href = "http://127.0.0.1:8000/window"
              } else {
                  console.log("not ok")
              }
          })
        ...
        comment(id) {
                this.commentpage = id;
                url = 'http://127.0.0.1:8000/api/getcomment'
                this.$http.get(url, {
                    params: {id}
                }).then(function(res) {
                    if (res.ok) {
        ...
        deletenote(noteid) {
            ...
            if (r == true) {
                url = 'http://127.0.0.1:8000/deletenote'
                this.$http.post(url, noteid, {
                    ...
                    if (res.ok) {
                        window.location.href = "http://127.0.0.1:8000/mynote" + "?page=1"
                    } else {
                        console.log("not ok")
                    }
                })
        ...
        onNote() {
            ...
            if (this.artical.bookname && this.artical.content) {
                var formData = new FormData()
                url = 'http://127.0.0.1:8000/onnote'
        ...
        gonumpage(x) {
            window.location.href = "http://127.0.0.1:8000/mynote" + "?page=" + x
        },
        outchange() {            
            this.flag = true;
        },
